...
% TRANSFORMER2WIMP Application M-file for TransformerTests.fig
%    FIG = TRANSFORMER2WIMP launch TransformerTests GUI.
%    TRANSFORMER2WIMP('callback_name', ...) invoke the named callback.

% Last Modified by GUIDE v2.0 24-May-2002 16:57:45
...
ax1=axes('Position',[0.05  .48  .9  .23]);
...
x2=3.0+[0.6  .65 .7     .80  0.9   1.0   1.1   1.2   1.25  1.375  ];
...
x22=[3.6  5.4  5.4];
y22=[0  0   .3];
xL2=[5.35  5.35  5.45  5.45  5.35];
yL2=[.3  .9  .9  .3  .3];
plot(x21, y21, 'b',x22, y22,'b')
plot(xL2, yL2,'b')
plot(x2, y2,'b')
plot(3+xs1,ys1,'b', 3+xs2,ys1, 'b', 3+xs3,ys1,'b', 3+xs4, ys1,'b') 



text(-.235,1.1625, '\circ', 'color', [0 0 1])
text(-.235,-.0375, '\circ', 'color', [0 0 1])
%text(-.3, .8, 'V_1', 'Fontsize', 8, 'color', [0 0 .675])
text(0, .35, 'R_{c1}', 'Fontsize', 8, 'color', [0 0 .675])
text(.8, .35, 'X_{m1}', 'Fontsize', 8, 'color', [0 0 .675])
text(.9, 1, 'R_1', 'Fontsize', 8, 'color', [0 0 .675])
text(1.65, 1.05, 'X_1', 'Fontsize', 8, 'color', [0 0 .675])
text(3+.9, 1, 'R_2', 'Fontsize', 8, 'color', [0 0 .675])
text(3+1.65, 1.05, 'X_2', 'Fontsize', 8, 'color', [0 0 .675])
text(5.00, .6, 'S_{Load}', 'Fontsize', 8, 'color', [0 0 .675])
...
% --------------------------------------------------------------------
function varargout = R1Text_Callback(h, eventdata, handles, varargin)
% --------------------------------------------------------------------
function varargout = X1Text_Callback(h, eventdata, handles, varargin)
% --------------------------------------------------------------------
function varargout = RC1Text_Callback(h, eventdata, handles, varargin)
% --------------------------------------------------------------------
function varargout = XM1Text_Callback(h, eventdata, handles, varargin)
% --------------------------------------------------------------------
function varargout = R2Text_Callback(h, eventdata, handles, varargin)
% --------------------------------------------------------------------
function varargout = X2Text_Callback(h, eventdata, handles, varargin)
...
% Transformer Equivalent Circuit (Given individual winding impedances)
...
R_1Handle=findobj(gcbf, 'Tag','R1Text');
R_1=eval(get(R_1Handle,'String'));
X_1Handle=findobj(gcbf, 'Tag','X1Text');
X_1=eval(get(X_1Handle,'String'));
R_2Handle=findobj(gcbf, 'Tag','R2Text');
R_2=eval(get(R_2Handle,'String'));
X_2Handle=findobj(gcbf, 'Tag','X2Text');
X_2=eval(get(X_2Handle,'String'));
R_C1Handle=findobj(gcbf, 'Tag','RC1Text');
R_C1=eval(get(R_C1Handle,'String'));
X_M1Handle=findobj(gcbf, 'Tag','XM1Text');
X_M1=eval(get(X_M1Handle,'String'));
   if StDwn == 1
   Rchv = R_C1; Xmhv = X_M1;
   Rclv = (Elv/Ehv)^2*R_C1; Xmlv = (Elv/Ehv)^2*X_M1;
   Rehv=R_1+ (Ehv/Elv)^2*R_2;
   Relv=R_2+ (Elv/Ehv)^2*R_1;
   Xehv=X_1+ (Ehv/Elv)^2*X_2;
   Xelv=X_2+ (Elv/Ehv)^2*X_1;
   Zelv=Relv+j*Xelv; 
   Zehv=Rehv+j*Xehv;
elseif StUp ==1
   Rclv = R_C1; Xmlv = X_M1;
   Rchv = (Ehv/Elv)^2*R_C1; Xmhv = (Ehv/Elv)^2*X_M1;
   Relv=R_1+ (Elv/Ehv)^2*R_2;
   Rehv=R_2+ (Ehv/Elv)^2*R_1;
   Xelv=X_1+ (Elv/Ehv)^2*X_2;
   Xehv=X_2+ (Ehv/Elv)^2*X_1;
   Zelv=Relv+j*Xelv
   Zehv=Rehv+j*Xehv
else, end
...
axes(handles.EqCktAxes2)    % Select the proper axes
hrst2=gca;
cla(hrst2)
%if Error == 1
  %plot(0, 0), axis off
