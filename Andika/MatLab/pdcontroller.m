function varargout = PdController(varargin)
global nump denp numh denh

% PdCONTROLLER Application M-file for PController.fig
%    FIG = PdCONTROLLER launch PController GUI.
%    PdCONTROLLER('callback_name', ...) invoke the named callback.
...
% This program is used for the design of a PD controller.
...
axes(handles.axesh4)    % Select the proper axes
hrst2=gca;
cla(hrst2)

text(0.05, .9, ['Controller: G_c(s) = ', num2str(Kp),' + ', num2str(KD), 's'], 'color', [0 0 0.62745])
if KD < 0 | Kp < 0 
